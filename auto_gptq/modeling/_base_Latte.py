...
class BaseGPTQForLatte(nn.Module, PushToHubMixin):
    layer_type: str = None
    layers_block_name: str = None
    temporal_layers_block_name: str = None
    ...
    def quantize(
        ...
        spatial_layers = get_module_by_name_prefix(self.model, self.layers_block_name)
        temporal_layers = get_module_by_name_prefix(self.model, self.temporal_layers_block_name)
        
        cur_layer_device = get_device(spatial_layers[0])
        data_device = cur_layer_device if cache_examples_on_gpu else CPU
        def store_input_hook(_, args, kwargs):
            # 0 hidden_states: torch.FloatTensor,
            # 1 attention_mask: Optional[torch.FloatTensor] = None,
            # 2 encoder_hidden_states: Optional[torch.FloatTensor] = None,
            # 3 encoder_attention_mask: Optional[torch.FloatTensor] = None,
            # 4 spatial_timestep: Optional[torch.LongTensor] = None,
            # 5 cross_attention_kwargs: Dict[str, Any] = None,
            # 6 class_labels: Optional[torch.LongTensor] = None,
            # 7 temporal_timestep: Optional[torch.LongTensor] = None,
            layer_input = []
            for inp in args:
                layer_input.append(move_to_device(inp, data_device))
            layer_inputs.append(layer_input[0])
            extra_inputs.append(layer_input[1:])
            raise ValueError

        force_layer_back_to_cpu = False
        if get_device(spatial_layers[0]) == CPU:
            spatial_layers[0] = spatial_layers[0].to(CUDA_0)
            temporal_layers[0] = temporal_layers[0].to(CUDA_0)
        ...
        text_encoder = text_encoder.to(CUDA_0)
        # TODO: make this optional, backporting https://github.com/huggingface/optimum/blob/main/optimum/gptq/quantizer.py
        handle = spatial_layers[0].register_forward_pre_hook(store_input_hook, with_kwargs=True)
        ...
        move_to_device(spatial_layers[0], CPU if force_layer_back_to_cpu else cur_layer_device)
        move_to_device(temporal_layers[0], CPU if force_layer_back_to_cpu else cur_layer_device)
        ...
        quantizers = {}
        for i in range(len(spatial_layers)):
            print(f"i={i}, input shape:{layer_inputs[0].shape}")
            logger.info(f"Start quantizing spatial layer {i + 1}/{len(spatial_layers)}")
            layer = spatial_layers[i]
            force_layer_back_to_cpu = False
            ...
            for names in inside_layer_modules:
                ...
                for j in range(num_batches):
                    layer_input = []
                    layer_inp, extra_inp = layer_inputs[j], extra_inputs[j]
                    layer_input = [move_to_device(inp, cur_layer_device) for inp in [layer_inp, *extra_inp]]
                ...
                for name in subset:
                    logger.info(f"Quantizing {name} in spatial layer {i + 1}/{len(spatial_layers)}...")
            ...
            spatial_layers[i] = move_to_device(layer, CPU if force_layer_back_to_cpu else cur_layer_device)
            del layer
            del gptq
            del layer_inputs
            ...
            # quant temporal transformers
            logger.info(f"Start quantizing temporal layer {i + 1}/{len(temporal_layers)}")
            layer = temporal_layers[i]
            force_layer_back_to_cpu = False
            if get_device(layer) == CPU:
                move_to_device(layer, CUDA_0)
                force_layer_back_to_cpu = True
            ...
            for names in inside_layer_modules:
                ...
                for j in range(num_batches):
                    layer_inp, extra_inp = layer_inputs[j], extra_inputs[j]
                    layer_input = [move_to_device(inp, cur_layer_device) for inp in [layer_inp, *extra_inp]]
                    hidden_states = rearrange(layer_input[0], '(b f) t d -> (b t) f d', b=self.model.input_batch_size).contiguous()
                    if i == 0:
                        hidden_states = hidden_states + self.model.temp_pos_embed.to(cur_layer_device)
                    layer_input[0] = hidden_states
                    layer_input[1] = None
                    layer_input[2] = None
                    layer_input[3] = None
                    layer_input[4] = layer_input[-1]
                    layer_input = layer_input[:-1]
                    layer(*layer_input)
                for h in handles:
                    h.remove()
                ...
                for name in subset:
                    logger.info(f"Quantizing {name} in temporal layer {i + 1}/{len(spatial_layers)}...")
                    scale, zero, g_idx = gptq[name].fasterquant(
                        percdamp=self.quantize_config.damp_percent,
                        group_size=self.quantize_config.group_size,
                        actorder=self.quantize_config.desc_act,
                    ...
                    quantizers[f"{self.temporal_layers_block_name}.{i}.{name}"] = (
                        gptq[name].quantizer.to(CPU if force_layer_back_to_cpu else cur_layer_device),
                        move_to_device(scale, CPU if force_layer_back_to_cpu else cur_layer_device),
                        move_to_device(zero, CPU if force_layer_back_to_cpu else cur_layer_device),
                        move_to_device(g_idx, CPU if force_layer_back_to_cpu else cur_layer_device),
            ...
            temporal_layers[i] = move_to_device(layer, CPU if force_layer_back_to_cpu else cur_layer_device)
            # breakpoint()
            del layer
            del gptq
            del layer_inputs
            ...
            for j in range(num_batches):
                layer_inputs[j] = rearrange(layer_inputs[j], '(b t) f d -> (b f) t d', b=self.model.input_batch_size).contiguous()
        pack_model(
            model=self.model,
    ...
    def from_pretrained(
        ...
        **model_init_kwargs,
        ...
        from AutoGPTQ.Latte.models import LatteT2V
        from AutoGPTQ.Latte.download import find_model
    ...
    def from_quantized(
        ...
        **kwargs,
        ...
        # == step4: (optional) inject optimized module == #
